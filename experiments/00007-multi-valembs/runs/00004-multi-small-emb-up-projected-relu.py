...
class GPT(nn.Module):
    def __init__(
        ...
        compressed_dim = next_multiple_of_n(model_dim // embed_dim_div, n=128)
        self.extra_embeds = nn.ModuleList(
            [nn.Embedding(vocab_size, compressed_dim) for _ in range(num_layers)]
        )
        self.decompressor = nn.Parameter(
            init_linear(torch.empty(model_dim, compressed_dim)).bfloat16()
        )
    ...
    def forward(
        ...
        for i in range(len(self.blocks)):
            ...
            x01 = norm(self.extra_embeds[i](input_seq)[None])
            x01 = F.relu(F.linear(x01, self.decompressor))
            x = self.blocks[i](
...
parser.add_argument("--embed-dim-div", "-d", type=int)
embed_dim_div = parser.parse_args().embed_dim_div
...
if master_process:
    ...
    path = f"../logs/00004-multi-small-emb-up-projected-relu-embed_dim_div{embed_dim_div}"
...
adam_param_groups = [
    ...
    dict(params=decompressor_params, lr=1 / 320),
