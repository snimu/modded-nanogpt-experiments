...
class GPT(nn.Module):
            ...
            torch.cat(
                [
                    ...
                    *[
                    ...
                    torch.zeros(len(skip_layers)), torch.tensor([1.0]),  # skip-to-head lambdas
                ]
            )
        )
    ...
    def forward(
        ...
        skip_lambdas = self.scalars[-len(self.skip_layers) - 1:]
        x = norm(x) * skip_lambdas[-1]
        for i, skip_layer in enumerate(self.skip_layers):
            x = x + norm(skip_connections[skip_layer]) * skip_lambdas[-i-2]
        if self.training:
...
skip_layers = random.sample([i for i in range(15)], num_skips)
...
if master_process:
    ...
    path = "../logs/8000-add-skip-random-order-"
