...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.scalars = nn.Parameter(torch.cat([
            ...
            *[torch.tensor([0.5, 0.5]) for _ in range(num_layers)], # SA lambdas
            torch.tensor([1.0, 0.0]),  # skip weights: x00 to lm_head
        ]))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x00_skip_lambdas = self.scalars[6 * len(self.blocks) : 6 * len(self.blocks) + 2]
        x = norm(x) * x00_skip_lambdas[0] + x00 * x00_skip_lambdas[1]
        if self.training:
...
if master_process:
    ...
    os.makedirs("../logs/10-skip-to-head-improved-skip-saving", exist_ok=True)
    logfile = f"../logs/10-skip-to-head-improved-skip-saving/{run_id_full}.txt"
    print(logfile)
