...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        self, x: Tensor, ve: list[Tensor] | None, block_mask: BlockMask, lambdas: Tensor
    ):
...
class GPT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.num_embs_per_ve = num_embs_per_ve
        ...
        self.value_embeds = nn.ModuleList(
            [
                nn.ModuleList([nn.Embedding(vocab_size, model_dim) for _ in range(num_embs_per_ve)])
        ...
        self.scalars = nn.Parameter(
            torch.cat(
                [
                    ...
                    *[
                        torch.empty(num_layers * (num_embs_per_ve + 1)).fill_(0.5)
                    ],  # SA lambdas
    ...
    def forward(
        ...
        ve = [
            [value_embed(input_seq) for value_embed in embs]
            for embs in self.value_embeds
        ]

        num_skipped = len(self.blocks) - 2 * len(self.value_embeds)
        ve = [valembs for valembs in ve] + [None] * num_skipped + [valembs for valembs in ve]
...
parser.add_argument("--num-ve", type=int)
parser.add_argument("--num-embs-per-ve", type=int)
cli_args = parser.parse_args()
num_ve = cli_args.num_ve
num_embs_per_ve = cli_args.num_embs_per_ve
...
model: nn.Module = GPT(
    ...
    num_ve=num_ve,
    num_embs_per_ve=num_embs_per_ve,
).cuda()
