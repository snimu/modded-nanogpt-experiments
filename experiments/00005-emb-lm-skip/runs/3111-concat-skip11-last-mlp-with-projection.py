...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, compressed_dim: int, max_seq_len: int):
        super().__init__()
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x = torch.cat([x, F.linear(skip_connections[11], self.compressor)], dim=-1)
...
if master_process:
    ...
    os.makedirs("../logs/3111-concat-skip11-last-mlp-with-projection", exist_ok=True)
    logfile = f"../logs/3111-concat-skip11-last-mlp-with-projection/{run_id_full}.txt"
    print(logfile)
...
model: nn.Module = GPT(
    ...
    compressed_dim=256, max_seq_len=max(args.train_seq_len, args.val_seq_len)
).cuda()
