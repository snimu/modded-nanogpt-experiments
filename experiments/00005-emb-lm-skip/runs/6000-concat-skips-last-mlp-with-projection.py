...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, layer_idx: int):
        ...
        # increase residual size in last MLP by concatenating previous activation
        self.mlp = MLP(dim, hdim=4*dim) if layer_idx != 15 else None
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, compressed_dim: int, skip_layer: int, max_seq_len: int):
        super().__init__()
        ...
        self.out_mlp = MLP(dim=model_dim+compressed_dim, hdim=model_dim+compressed_dim)
        self.compressor = nn.Parameter(init_linear(torch.empty(compressed_dim, model_dim)).bfloat16())
        # there are only 50257 unique GPT-2 tokens; we extend to nearest multiple of 128 for efficiency.
        # suggested to me by @Grad62304977. this originates from Karpathy's experiments.
        self.lm_head_w = nn.Parameter(torch.zeros(next_multiple_of_n(vocab_size, n=128), model_dim + compressed_dim))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        for i in range(len(self.blocks)):
            ...
            if i == len(self.blocks) - 1:
                x_concat = skip_connections[11]
            x = self.blocks[i](x, ve[i], x00, x01, block_masks[i], lambdas[i], sa_lambdas[i], x_concat)
            skip_connections.append(x)

        x = torch.cat([x, F.linear(skip_connections[self.skip_layer], self.compressor)], dim=-1)
        x = norm(x + self.out_mlp(norm(x)))
        if self.training:
...
parser.add_argument("--compressed-dim", type=int, default=128)
cli_args = parser.parse_args()
skip_layer = cli_args.skip_layer
compressed_dim = cli_args.compressed_dim
...
if master_process:
    ...
    os.makedirs(f"../logs/6000-concat-skip{skip_layer}-last-mlp-with-projection{compressed_dim}", exist_ok=True)
    logfile = f"../logs/6000-concat-skip{skip_layer}-last-mlp-with-projection{compressed_dim}/{run_id_full}.txt"
    print(logfile)
...
model: nn.Module = GPT(
    ...
    compressed_dim=compressed_dim, skip_layer=skip_layer,
...
hidden_matrix_params2 = sorted((p for p in model.out_mlp.parameters() if p.ndim >= 2), key=lambda x: x.size(), reverse=True)
embed_params = [
...
compressor_params: list[nn.Parameter] = [model.compressor]
# sanity check
params_collections = [hidden_matrix_params, hidden_matrix_params2, embed_params, scalar_params, head_params, compressor_params]
optimized_parameters_set = {p for params in params_collections for p in params}
assert optimized_parameters_set == {*model.parameters()}
assert len(optimized_parameters_set) == sum(len(lst) for lst in params_collections)

# init the optimizer(s)
adam_param_groups = [dict(params=head_params, lr=1/320), dict(params=compressor_params, lr=1/320), dict(params=embed_params, lr=0.3), dict(params=scalar_params, lr=0.015)]
...
optimizer3 = Muon(hidden_matrix_params2, lr=0.025, momentum=0.95, rank=rank, world_size=world_size)
