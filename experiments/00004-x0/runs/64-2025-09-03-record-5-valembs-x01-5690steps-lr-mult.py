...
class Hyperparameters:
    ...
    # optimization
    num_iterations = 5790 # number of iterations to run
...
if master_process:
    ...
    os.makedirs("../logs/64-2025-09-03-record-5-valembs-x01-5690steps-lr-mult", exist_ok=True)
    logfile = f"../logs/64-2025-09-03-record-5-valembs-x01-5690steps-lr-mult/{run_id_full}.txt"
    print(logfile)
...
lr_mult = 1.3  # the fewer the steps, the higher the learning rate must be. Single dial to tune.
# init the optimizer(s)
adam_param_groups = [dict(params=head_params, lr=1/320*lr_mult), dict(params=embed_params, lr=0.3*lr_mult), dict(params=scalar_params, lr=0.015*lr_mult)]
# small adam epsilon by @YouJiacheng. this is an alternate method of fixing the world_size dependence
# discovered by @fernbear.bsky.social https://x.com/hi_tysam/status/1879692937589875094
optimizer1 = torch.optim.AdamW(adam_param_groups, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0, fused=True)
optimizer2 = Muon(hidden_matrix_params, lr=0.025*lr_mult, momentum=0.95, rank=rank, world_size=world_size)
