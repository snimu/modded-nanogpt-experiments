...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.scalars = nn.Parameter(torch.cat([
            ...
            *[torch.tensor([0.5, 0.5]) for _ in range(num_layers)], # SA lambdas
            torch.tensor([1.0, 0.0, 0.0]),  # skip weights: x00 to lm_head
        ]))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        for i in range(len(self.blocks)):
            ...
            skip_connections.append(x if i in skip_map.values() else None)

        x00_skip_lambdas = self.scalars[6 * len(self.blocks) : 6 * len(self.blocks) + 3]
        x = norm(x) * x00_skip_lambdas[0] + x00 * x00_skip_lambdas[1] + x01 * x00_skip_lambdas[2]
        if self.training:
...
if master_process:
    ...
    os.makedirs("../logs/11-skip-to-head-improved-skip-saving-x00-and-x01", exist_ok=True)
    logfile = f"../logs/11-skip-to-head-improved-skip-saving-x00-and-x01/{run_id_full}.txt"
    print(logfile)
