...
class GPT(nn.Module):
    def __init__(
        ...
        # Projection for skip to output
        self.skip_projection = nn.Parameter(init_linear(torch.empty(model_dim, model_dim)).bfloat16())
    ...
    def forward(
        ...
        skip_connection = norm(F.linear(F.relu(skip_connections[11]), self.skip_projection))
        x = norm(x) * skip_lambdas[0] + skip_connection * skip_lambdas[1]
        if self.training:
...
if master_process:
    ...
    path = "../logs/7012-add-skip11-record-from-updated-record-relu-linear"
...
skip_projection_params = [model.skip_projection]
scalar_params = [model.scalars]
head_params: list[nn.Parameter] = [model.lm_head_w]
# sanity check
params_collections = [hidden_matrix_params, embed_params, scalar_params, head_params, skip_projection_params]
...
adam_param_groups = [
    ...
    dict(params=skip_projection_params, lr=1 / 320),
