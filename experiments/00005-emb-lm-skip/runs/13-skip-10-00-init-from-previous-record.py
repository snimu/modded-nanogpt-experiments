...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        emb_lmhead_skip_lambdas = self.scalars[5*len(self.blocks):5*len(self.blocks)+2]
        ...
        x = norm(x) * emb_lmhead_skip_lambdas[0] + x0 * emb_lmhead_skip_lambdas[1]
        if self.training:
...
if master_process:
    ...
    os.makedirs("../logs/13-skip-10-00-init-from-previous-record", exist_ok=True)
    logfile = f"../logs/13-skip-10-00-init-from-previous-record/{run_id_full}.txt"
    print(logfile)
