...
def smear_embeddings(
    ...
    smear_gate_weight: nn.Parameter,
    ...
    return norm(torch.cat(
        [x[:, :1], x[:, 1:] + smear_gate_out[:, 1:] * F.linear(F.relu(x[:, :-1]), smear_trafo_weight)],
...
class GPT(nn.Module):
    def __init__(
        ...
        self.smear_gate_weight = nn.Parameter(init_linear(torch.empty(1, model_dim)).bfloat16())
        self.smear_mlp = MLP(model_dim, expansion_factor=0.5)
        self.smear_trafo_weight = nn.Parameter(init_linear(torch.empty(1, model_dim)).bfloat16())
    ...
    def forward(
        ...
        x = smear_embeddings(
            ...
            smear_gate_weight=self.smear_gate_weight,
...
if master_process:
    ...
    path = f"../logs/0010-mtp-with-trafo-smear_layer{smear_layer}"
...
smear_weight_params: list[nn.Parameter] = [model.smear_gate_weight, model.smear_trafo_weight]
