...
Move the value embeddings from layers 13-15 to 12-14 (still shared with 0-2).
Add an additional shared one to layers 3 and 15.
"""
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.value_embeds = nn.ModuleList([nn.Embedding(vocab_size, model_dim) for _ in range(7)])
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        ve = [ve[0], ve[1], ve[2], ve[3], ve[4], ve[5], ve[6]] + [None] * (len(self.blocks) - 14) + [ve[0], ve[1], ve[2], ve[3], ve[4], ve[5], ve[6]]
...
if master_process:
    ...
    os.makedirs("../logs/18-new-valemb-3-12-4-13-5-14-6-15", exist_ok=True)
    logfile = f"../logs/18-new-valemb-3-12-4-13-5-14-6-15/{run_id_full}.txt"
    print(logfile)
