...
class Rotary(nn.Module):
    ...
    def forward(self, x_BTHD: Tensor, rot_lambda: Tensor):
        T = x_BTHD.shape[1]
        assert self.theta.size(0) >= T
        theta = self.theta[:T] * rot_lambda.to(self.theta.dtype)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, block_mask: BlockMask, lambdas: Tensor, rot_lambdas: Tensor):
        ...
        q, k = self.rotary(q, rot_lambdas[0]), self.rotary(k, rot_lambdas[1])
        v = norm(v)
        if ve is not None:
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.attn is not None:
            x = x + self.attn(x, ve, block_mask, sa_lambdas, rot_lambdas)
        x = x + self.mlp(norm(x))
        return x
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.scalars = nn.Parameter(torch.cat([
            ...
            *[torch.tensor([1.0, 1.0]) for _ in range(num_layers)], # rotary lambdas
        ]))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        rot_lambdas = self.scalars[5 * len(self.blocks): 7 * len(self.blocks)].view(-1, 2)
        for i in range(len(self.blocks)):
            if i in skip_map:
                x = x + skip_weights[skip_map[i]] * skip_connections[skip_map[i]]
            x = self.blocks[i](x, ve[i], x0, block_masks[i], lambdas[i], sa_lambdas[i], rot_lambdas[i])
...
if master_process:
    ...
    os.makedirs("../logs/2-measure-rope-via-theta", exist_ok=True)
    logfile = f"../logs/2-measure-rope-via-theta/{run_id_full}.txt"
    print(logfile)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        rot_lambdas = model.scalars[5 * len(model.blocks): 6 * len(model.blocks)]
        print0(f"step:{step}/{train_steps} rot_lambdas_q:{rot_lambdas[:, 0].tolist()}", console=True)
        print0(f"step:{step}/{train_steps} rot_lambdas_k:{rot_lambdas[:, 1].tolist()}", console=True)
        model.train()
