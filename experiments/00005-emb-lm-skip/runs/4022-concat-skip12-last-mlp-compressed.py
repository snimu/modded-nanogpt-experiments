...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, expansion_factor=4) if layer_idx != 15 else MLP(dim + compressed_dim, expansion_factor=1)
        self.compressor = nn.Parameter(init_linear(torch.empty(compressed_dim, dim)).bfloat16()) if layer_idx == 15 else None
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, compressed_dim: int, max_seq_len: int):
        ...
        self.blocks = nn.ModuleList([Block(model_dim, compressed_dim, num_heads, max_seq_len, i) for i in range(num_layers)])
...
if master_process:
    ...
    os.makedirs("../logs/4022-concat-skip12-last-mlp-compressed", exist_ok=True)
    logfile = f"../logs/4022-concat-skip12-last-mlp-compressed/{run_id_full}.txt"
    print(logfile)
...
model: nn.Module = GPT(
    ...
    compressed_dim=384, max_seq_len=max(args.train_seq_len, args.val_seq_len)
).cuda()
