...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, layer_idx: int):
        ...
        self.mlp = MLP(dim, expansion_factor=4) if layer_idx != 15 else MLP(2 * dim, expansion_factor=1)

    def _mlp_with_norm(self, z: Tensor) -> Tensor:
        return self.mlp(norm(z))

    def _chunked_checkpoint_mlp(self, x: Tensor, chunk: int = 2048) -> Tensor:
        outs = []
        for s in range(0, x.size(1), chunk):
            xs = x[:, s:s+chunk]
            ys = checkpoint(self._mlp_with_norm, xs, use_reentrant=False)
            outs.append(ys)
    ...
    def forward(
        ...
        if x_concat is not None:
            ...
            x = x + self._chunked_checkpoint_mlp(x, chunk=2048)
        else:
            x = x + self.mlp(norm(x))
        return x
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.lm_head_w = nn.Parameter(torch.zeros(next_multiple_of_n(vocab_size, n=128), model_dim * 2))
...
if master_process:
    ...
    os.makedirs("../logs/3000-concat-x00-last-mlp-with-seq-chunking", exist_ok=True)
    logfile = f"../logs/3000-concat-x00-last-mlp-with-seq-chunking/{run_id_full}.txt"
    print(logfile)
