...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.scalars = nn.Parameter(torch.cat([
            ...
            *[torch.tensor([0.5, 0.5]) for _ in range(num_layers)], # SA lambdas
            torch.tensor([1.0, 0.0]),  # x & x0 skip lambdas before lm-head
        ]))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x02 = norm(self.embed3(input_seq)[None])
        ...
        emb_lmhead_skip_lambdas = self.scalars[7*len(self.blocks):7*len(self.blocks)+2]
        ...
        x = norm(x) * emb_lmhead_skip_lambdas[0] + x00 * emb_lmhead_skip_lambdas[1]
        if self.training:
...
class Hyperparameters:
    ...
    # optimization
    num_iterations = 5910 # number of iterations to run
...
if master_process:
    ...
    os.makedirs("../logs/15-skip-from-previous-record-x00-x01-x02", exist_ok=True)
    logfile = f"../logs/15-skip-from-previous-record-x00-x01-x02/{run_id_full}.txt"
    print(logfile)
