...
class GPT(nn.Module):
    ...
    def forward(
        ...
        rolling_lambdas = self.scalars[6 * len(self.blocks) : 8 * len(self.blocks)].view(
            -1, 2
        )
        for i in range(len(self.blocks)):
            ...
            if i > 2:
                l1, l2 = rolling_lambdas[i][0], rolling_lambdas[i][1]
                x = x * l1 + skip_connections[-2] * l2
            x = self.blocks[i](
...
if master_process:
    ...
    path = "../logs/8001-add-skip-rolling"
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        rolling_lambdas = model.scalars[6 * len(model.blocks) : 8 * len(model.blocks)].view(
            -1, 2
        ).tolist()
        print0(
            f"step:{step}/{train_steps} val_loss:{val_loss:.6f} train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms/max(step, 1):.2f}ms rolling_lambdas:{rolling_lambdas}",
            console=True,
        )
        model.train()
