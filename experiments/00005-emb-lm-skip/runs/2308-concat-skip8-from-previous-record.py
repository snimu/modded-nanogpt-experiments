...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x = torch.cat([norm(x), norm(skip_connections[8])], dim=-1)
        if self.training:
...
if master_process:
    ...
    os.makedirs("../logs/2308-concat-skip8-from-previous-record", exist_ok=True)
    logfile = f"../logs/2308-concat-skip8-from-previous-record/{run_id_full}.txt"
    print(logfile)
