...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        ve = [ve[0], ve[1], ve[2], ve[3], ve[4]] + [None] * (len(self.blocks) - 6) + [ve[0], ve[1], ve[2], ve[3], ve[4]]
        ...
        x = torch.cat([norm(x), norm(skip_connections[12])], dim=-1)
        if self.training:
...
if master_process:
    ...
    os.makedirs("../logs/2314-concat-skip14-from-previous-record", exist_ok=True)
    logfile = f"../logs/2314-concat-skip14-from-previous-record/{run_id_full}.txt"
    print(logfile)
