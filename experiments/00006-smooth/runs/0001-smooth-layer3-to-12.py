...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, layer_idx: int, expansion_factor: int = 4):
        ...
        self.mlp = MLP(dim, expansion_factor=expansion_factor)
...
class Smoother(nn.Module):
    def __init__(self, dim: int):
        super().__init__()
        self.transform = MLP(dim, expansion_factor=0.5)
        self.smooth = nn.Parameter(init_linear(torch.empty(1, dim)).bfloat16())
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.smoother = Smoother(model_dim)
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x = x01 = norm(self.embed2(input_seq)[None])
        ...
        smooth_from, smooth_to = 3, 12
        for i in range(len(self.blocks)):
            if i in skip_map:
                x = x + skip_weights[skip_map[i]] * skip_connections[skip_map[i]]
            if i == smooth_to:
                x = self.smoother(x, skip_connections[smooth_from])
...
if master_process:
    ...
    os.makedirs("../logs/0001-smooth-layer3-to-12", exist_ok=True)
    logfile = f"../logs/0001-smooth-layer3-to-12/{run_id_full}.txt"
    print(logfile)
