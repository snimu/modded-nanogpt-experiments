...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        emb_lmhead_skip_lambdas = self.scalars[6*len(self.blocks):6*len(self.blocks)+2]
...
if master_process:
    ...
    os.makedirs("../logs/17-skip-from-previous-record-x00-x01", exist_ok=True)
    logfile = f"../logs/17-skip-from-previous-record-x00-x01/{run_id_full}.txt"
    print(logfile)
