...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, block_mask: BlockMask, lambdas: Tensor, rot_lambda: Tensor):
        ...
        q, k = self.rotary(q, rot_lambda), self.rotary(k, rot_lambda)
        v = norm(v)
        if ve is not None:
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.attn is not None:
            x = x + self.attn(x, ve, block_mask, sa_lambdas, rot_lambda)
        x = x + self.mlp(norm(x))
        return x
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.scalars = nn.Parameter(torch.cat([
            ...
            *[torch.tensor([0.5, 0.5]) for _ in range(num_layers)], # SA lambdas
            torch.ones(num_layers), # rotary lambdas
        ]))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        rot_lambdas = self.scalars[5 * len(self.blocks): 6 * len(self.blocks)]
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        print0(f"step:{step}/{train_steps} rot_lambdas:{rot_lambdas.tolist()}", console=True)
        model.train()
