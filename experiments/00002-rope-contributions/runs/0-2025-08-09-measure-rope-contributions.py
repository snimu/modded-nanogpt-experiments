...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, block_mask: BlockMask, lambdas: Tensor, qrope_lambdas: Tensor, krope_lambdas: Tensor):
        ...
        qsum, ksum = qrope_lambdas.sum(), krope_lambdas.sum()
        q = q * qrope_lambdas[1] / qsum + self.rotary(q) * qrope_lambdas[0] / qsum
        k = k * krope_lambdas[1] / ksum + self.rotary(k) * krope_lambdas[0] / ksum
        v = norm(v)
        if ve is not None:
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, x0: Tensor, block_mask: BlockMask, lambdas: Tensor, sa_lambdas: Tensor, qrope_lambdas: Tensor, krope_lambdas: Tensor):
        x = lambdas[0] * x + lambdas[1] * x0
        if self.attn is not None:
            x = x + self.attn(x, ve, block_mask, sa_lambdas, qrope_lambdas, krope_lambdas)
        x = x + self.mlp(norm(x))
        return x
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.scalars = nn.Parameter(torch.cat([
            ...
            *[torch.tensor([1.0, 0.0]) for _ in range(num_layers)], # q-rope-lambdas
            *[torch.tensor([1.0, 0.0]) for _ in range(num_layers)], # k-rope-lambdas
        ]))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        qrope_lambdas = self.scalars[5 * len(self.blocks): 7 * len(self.blocks)].view(-1, 2)
        krope_lambdas = self.scalars[7 * len(self.blocks): 9 * len(self.blocks)].view(-1, 2)
        for i in range(len(self.blocks)):
            if i in skip_map:
                x = x + skip_weights[skip_map[i]] * skip_connections[skip_map[i]]
            x = self.blocks[i](x, ve[i], x0, block_masks[i], lambdas[i], sa_lambdas[i], qrope_lambdas[i], krope_lambdas[i])
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        qrope_lambdas = model.scalars[5 * len(model.blocks): 7 * len(model.blocks)].view(-1, 2)
        krope_lambdas = model.scalars[7 * len(model.blocks): 9 * len(model.blocks)].view(-1, 2)
        ...
        print0(f"step:{step}/{train_steps} qrope_lambda_0:{qrope_lambdas[:, 0].tolist()}", console=True)
        print0(f"step:{step}/{train_steps} qrope_lambda_1:{qrope_lambdas[:, 1].tolist()}", console=True)
        print0(f"step:{step}/{train_steps} krope_lambda_0:{krope_lambdas[:, 0].tolist()}", console=True)
        print0(f"step:{step}/{train_steps} krope_lambda_1:{krope_lambdas[:, 1].tolist()}", console=True)
        model.train()
