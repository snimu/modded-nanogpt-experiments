...
def smear_embeddings(
    ...
    x_smear = x_smear[1:, :smear_weight.size(-1)]
...
class GPT(nn.Module):
    def __init__(
        ...
        self.smear_weight_in_1 = nn.Parameter(init_linear(torch.empty(1, 12)).bfloat16())
        self.smear_weight_in_2 = nn.Parameter(init_linear(torch.empty(1, 12)).bfloat16())
...
if master_process:
    ...
    path = f"../logs/0005-mtp-and-smear-inputs-smear_layer{smear_layer}"
...
smear_weight_in_params: list[nn.Parameter] = [model.smear_weight_in_1, model.smear_weight_in_2]
smear_mlp_params: list[nn.Parameter] = [p for p in model.smear_mlp.parameters() if p.ndim >= 2]
# sanity check
params_collections = [hidden_matrix_params, embed_params, scalar_params, head_params, smear_mlp_params, smear_weight_params, smear_weight_in_params]
...
inner_smear_mlp_optim = Muon(
    smear_weight_in_params, lr=0.03, momentum=0.95, update_smoothing=0.2, rank=rank, world_size=world_size
)
