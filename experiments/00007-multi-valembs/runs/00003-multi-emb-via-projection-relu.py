...
class GPT(nn.Module):
    ...
    def forward(
        ...
        for i in range(len(self.blocks)):
            ...
            embd = F.relu(F.linear(x00, self.embed_modifiers[i]))
            x = self.blocks[i](
                x, ve[i], embd, block_masks[i], lambdas[i], sa_lambdas[i]
            )
...
if master_process:
    ...
    path = "../logs/00003-multi-emb-via-projection-relu"
