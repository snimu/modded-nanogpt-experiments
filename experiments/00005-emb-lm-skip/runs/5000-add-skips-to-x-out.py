...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int, skip_layer: int):
        ...
        self.scalars = nn.Parameter(torch.cat([
            ...
            *[torch.tensor([0.5, 0.5]) for _ in range(num_layers)], # SA lambdas
            torch.tensor([1.0, 0.0]),  # skip lambdas
        ]))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x = norm(x) * skip_lambdas[0] + skip_connections[skip_layer] * skip_lambdas[1]
        if self.training:
...
if master_process:
    ...
    os.makedirs(f"../logs/5000-add-skip{skip_layer}-to-x-out", exist_ok=True)
    logfile = f"../logs/5000-add-skip{skip_layer}-to-x-out/{run_id_full}.txt"
    print(logfile)
